...
async function I(e, m) {
  ...
  }), r = 0, s = i.audio.sample_rate, d = i.inference.noise_scale, S = i.inference.length_scale, U = i.inference.noise_w, k = await (await f(`${u}/${n}`, m)).arrayBuffer(), y = await _.InferenceSession.create(k, {
    executionProviders: navigator.gpu ? ["webgpu"] : void 0
  }), w = {
    input: new _.Tensor("int64", t, [1, t.length]),
    input_lengths: new _.Tensor("int64", [t.length]),
    scales: new _.Tensor("float32", [d, S, U])
  };
  Object.keys(i.speaker_id_map).length && Object.assign(w, { sid: new _.Tensor("int64", [r]) });
  const {
    output: { data: E }
  } = await y.run(w);
  return new Blob([D(E, 1, s)], { type: "audio/x-wav" });
}
async function f(e, m) {
  let n = await b(e);
  return n || (n = await g(e, m), await p(e, n)), n;
}
async function F(e, m) {
  const n = c[e], o = [`${u}/${n}`, `${u}/${n}.json`];
  await Promise.all(
    o.map(async (a) => {
      p(a, await g(a, a.endsWith(".onnx") ? m : void 0));
    })
  );
}
async function L(e) {
